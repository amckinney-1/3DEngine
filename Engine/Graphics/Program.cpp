...
namespace nEngine
{
	...
	bool Program::Load(const std::string& filename, void* data)
	{
		...
		{
			auto vshader = engine->Get<nEngine::ResourceSystem>()->Get<nEngine::Shader>(vertex_shader, (void*)GL_VERTEX_SHADER);
			AddShader(vshader);
		}
		...
		{
			auto fshader = engine->Get<nEngine::ResourceSystem>()->Get<nEngine::Shader>(fragment_shader, (void*)GL_FRAGMENT_SHADER);
			AddShader(fshader);
		}
	...
	void Program::SetUniform(const std::string& name, glm::vec2& v2)
	{
		GLint uniform = GetUniform(name);
		glUniform2f(uniform, v2.x, v2.y);
	}

	void Program::SetUniform(const std::string& name, glm::vec3& v3)
	{
		GLint uniform = GetUniform(name);
		glUniform3f(uniform, v3.x, v3.y, v3.z);
	}

	void Program::SetUniform(const std::string& name, glm::vec4& v4)
	{
		GLint uniform = GetUniform(name);
		glUniform4f(uniform, v4.x, v4.y, v4.z, v4.w);
	}

	void Program::SetUniform(const std::string& name, glm::mat4& mx4)
	{
		GLint uniform = GetUniform(name);
		glUniformMatrix4fv(uniform, 1, GL_FALSE, glm::value_ptr(mx4));
	}

	void Program::SetUniform(const std::string& name, glm::mat3& mx3)
	{
		GLint uniform = GetUniform(name);
